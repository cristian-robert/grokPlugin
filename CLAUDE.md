...
1. **READ-ONLY IS NON-NEGOTIABLE.** Grok reviews; it never edits. Enforce in depth — every layer stays even if another seems sufficient:
   1. **Claude side:** the command sets `disallowed-tools: Edit Write NotebookEdit` (removes them while the command runs — `allowed-tools` only pre-approves, it does NOT restrict); command text forbids fixing findings.
   ...
   5. **Isolation:** env disables all Claude/Cursor compat, memory, subagents, AND sets `HOME` to an empty temp dir (`GROK_HOME` stays real) — the compat switches alone still let Claude *plugins* (hooks + MCP) load. Verified before every run via `grok inspect --json`; anything active → refuse.
   6. **Post-run integrity check:** fingerprint the repo (HEAD, refs, index, status, diff, untracked contents) before and after; any change → loud `GUARDRAIL VIOLATION`, non-zero exit, list changed paths. **Never auto-revert** (could destroy concurrent user edits).
