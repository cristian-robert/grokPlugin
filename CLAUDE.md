...
1. **READ-ONLY IS NON-NEGOTIABLE.** Grok reviews; it never edits. Enforce in depth — every layer stays even if another seems sufficient:
   ...
   5. **Isolation:** grok gets an allowlisted env only (no inherited credentials or `GROK_*` overrides) with all Claude/Cursor compat, memory, subagents, auto-update off, AND `HOME` set to an empty temp dir (`GROK_HOME` stays real) — the compat switches alone still let Claude *plugins* (hooks + MCP) load. Verified before every run via `grok inspect --json`; active plugin/hook/MCP/LSP → refuse; repo-supplied instructions/skills/permission rules → loud warning in the header.
   ...
   6. **Post-run integrity check:** fingerprint the repo (HEAD, refs, index + skip-worktree/assume-unchanged file bytes, `.git` HEAD/config/info/hooks/modules, submodules recursively, modified + untracked file bytes, ignored files by size+mtime) and grok-home persistence spots (plugins, skills, agents, hooks, bin, config) before and after; a fingerprint that throws afterwards counts as a violation; every plugin git call pins `core.fsmonitor=false` and resolves `git` to an absolute path (Windows searches the cwd first); any change → loud `GUARDRAIL VIOLATION`, non-zero exit, list changed paths. **Never auto-revert** (could destroy concurrent user edits).
