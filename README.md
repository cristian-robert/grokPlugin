...
6. **Changes during the review:** the repository is fingerprinted before and after the run: HEAD, refs, index, `.git/HEAD`, `.git/config`, `.git/info/`, `.git/hooks/`, submodules, the bytes of every modified and untracked file, and the size and modification time of ignored files (e.g. `node_modules/`, `.env`). Anything that changed while Grok was reviewing (normally your own edits) is listed at the end of the review, which is still returned in full. Nothing is ever reverted. The plugin's own git calls pin `core.fsmonitor=false`, so a planted fsmonitor can't run during the check.
...
- The change list can't tell your edits apart from anything else that wrote to the repo during the run; with Grok's read-only tools that is normally you.
